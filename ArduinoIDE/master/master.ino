...
const char* node2_host = "192.168.204.136";
int angle2 = 0;
float distance2 = 0;
  ...
  function fetchData(radar) {
    ...
    fetch(radar.endpoint)
      ...
      .then(data => {
        ...
        radar.dot.dist = -1; // Ẩn dot
      })
      .finally(() => {
        radar.ctx.canvas.style.opacity = '1';
      });
  }
...
void fetchNode2Data() {
  if (millis() - lastFetchNode2 < 1000) return;

  HTTPClient http;
  WiFiClient client; 

  http.setTimeout(1500);
  String url = "http://" + String(node2_host) + "/data";
  ...
  if (http.begin(client, url)) {
    ...
    if (httpCode == HTTP_CODE_OK) {
      ...
      if (!error) {
        angle2 = doc["angle"];
        distance2 = doc["distance"];
        Serial.printf("[Node2] Angle: %d | Distance: %.2f\n", angle2, distance2);
      } else {
...
void setup() {
  ...
  server.on("/slave", []() {
    HTTPClient http;
    WiFiClient client;
    http.begin(client, "http://192.168.204.136/data");
    int httpCode = http.GET();
    if (httpCode == 200) {
      String payload = http.getString();
